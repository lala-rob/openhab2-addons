...
public class MilightLedHandler extends BaseThingHandler {
        ...
        if (command instanceof RefreshType) {
            switch (channelUID.getId()) {
                case MilightBindingConstants.CHANNEL_COLOR:
                    updateState(channelUID, new HSBType(new DecimalType(state.hue), new PercentType(100),
                            new PercentType(state.brightness)));
                    break;
                ...
                case MilightBindingConstants.CHANNEL_TEMP:
                    updateState(channelUID, new DecimalType(state.colorTemperature));
                    break;
                case MilightBindingConstants.CHANNEL_SPEED:
                    updateState(channelUID, new DecimalType(state.animationSpeed));
                    break;
                case MilightBindingConstants.CHANNEL_ANIMATION_MODE:
                    updateState(channelUID, new DecimalType(state.ledMode));
                    break;
            }
            return;
        }
        ...
        switch (channelUID.getId()) {
            ...
            case MilightBindingConstants.CHANNEL_TEMP: {
                if (command instanceof OnOffType) {
                    ...
                    if (id == IncreaseDecreaseType.INCREASE) {
                        bulbCom.changeColorTemperature(1, state);
            ...
            case MilightBindingConstants.CHANNEL_SPEED: {
                if (command instanceof IncreaseDecreaseType) {
                    ...
                    DecimalType d = (DecimalType) command;
                    bulbCom.setAnimationSpeed(d.intValue(), state);
                } else {
                    logger.error("CHANNEL_SPEED channel only supports IncreaseDecreaseType/DecimalType");
                }
                break;
            }
            case MilightBindingConstants.CHANNEL_ANIMATION_MODE: {
    ...
    public void initialize() {
        ...
        if (getBridge() != null) {
            ...
            MilightV6SessionManager session = brHandler.getSessionManager();

            if (thing.getThingTypeUID().equals(MilightBindingConstants.RGB_THING_TYPE)) {
                ...
                bulbCom = new MilightV6RGB_IBOX(com, session);
            } else if (thing.getThingTypeUID().equals(MilightBindingConstants.RGB_CW_WW_THING_TYPE)) {
                bulbCom = new MilightV6RGB_CW_WW(com, session, zone);
            } else if (thing.getThingTypeUID().equals(MilightBindingConstants.RGB_W_THING_TYPE)) {
                bulbCom = new MilightV6RGB_W(com, session, zone);
            } else {
